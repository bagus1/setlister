  ...
  async (req, res) => {
    try {
      ...
      if (!albumCheck.allowed) {
        req.flash(
          "error",
          albumCheck.message + ' <a href="/pricing">View Plans</a>'
        );
        return res.redirect(`/bands/${bandId}`);
      }
...
router.put("/:bandId/albums/:albumId", requireAuth, async (req, res) => {
  try {
    ...
    if (isPublished && !rightsConfirmed) {
      return res
        .status(400)
        .json({ error: "Rights confirmation required to publish album" });
    }
...
router.delete(
  "/:bandId/albums/:albumId/tracks/:trackId",
  requireAuth,
  async (req, res) => {
    try {
      const bandId = parseInt(req.params.bandId);
...
router.post(
  "/:bandId/albums/:albumId/tracks/reorder",
  requireAuth,
  async (req, res) => {
    try {
      const bandId = parseInt(req.params.bandId);
...
router.post(
  "/:bandId/albums/:albumId/tracks/:trackId/audio",
  requireAuth,
  async (req, res) => {
    try {
      const bandId = parseInt(req.params.bandId);
...
router.post(
  "/:bandId/albums/:albumId/tracks/:trackId/upload",
  requireAuth,
...
router.delete("/:bandId/albums/:albumId", requireAuth, async (req, res) => {
  try {
    ...
    console.log(
      `Deleted ${deleted} files for album ${albumId}, ${errors} errors`
    );
