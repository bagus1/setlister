...
deploy_to_demo() {
    ...
    if [ "$USER_COUNT" = "0" ] || [ "$USER_COUNT" = "" ]; then
        ...
        for table in users bands artists vocalists songs gig_documents setlists medleys band_members song_artists band_songs setlist_sets setlist_songs medley_songs band_invitations password_resets links; do
            ...
            ssh "$HOST_USER@$HOST_DOMAIN" "cd $DEMO_PATH && export DB_PASSWORD='$DB_PASSWORD' && export DB_USER='$DB_USER' && export DB_NAME='$DB_NAME' && [ -f 'migration-output/$table.sql' ] && PGPASSWORD=\$DB_PASSWORD psql -h localhost -U \$DB_USER -d \$DB_NAME -f 'migration-output/$table.sql' || echo 'No migration file for $table'" || {
