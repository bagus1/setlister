...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      // Compute preview waveform path (if generated by server)
      let previewWaveformPath = null;
      try {
        if (recording && recording.filePath) {
          const base = path
            .basename(recording.filePath)
            .replace(/\.[^/.]+$/, "");
          const candidate = `/uploads/waveforms/preview-${base}.dat`;
          const abs = path.join(__dirname, "..", "public", candidate);
          if (fs.existsSync(abs)) {
            previewWaveformPath = candidate;
          }
        }
      } catch (e) {
        // ignore
